    ...
    <div className="flex flex-col w-full bg-[#010410]">
      ...
      <SectionWrapper className="relative bg-black/40 border-y border-white/[0.03] overflow-hidden">
         ...
         <div className="absolute inset-0 pointer-events-none">
            ...
            <div className="absolute top-0 left-1/4 w-[350px] h-[350px] bg-cyan-500/10 blur-[100px] rounded-full" />
            <div className="absolute bottom-0 right-1/4 w-[350px] h-[350px] bg-blue-500/8 blur-[100px] rounded-full" />
         </div>
         ...
         <div className="flex flex-col items-center text-center mb-16 relative">
            <motion.h3
               ...
               viewport={{ once: true }}
               className="text-cyan-400 font-black tracking-[0.6em] uppercase text-[11px] mb-8"
            >Global Infrastructure Scaling</motion.h3>
            <motion.h2
            ...
            <motion.p
               ...
               viewport={{ once: true }}
               className="text-white/40 font-black tracking-[0.5em] uppercase text-xs mt-6 italic"
            >Projected Digital Health Market CAP</motion.p>
            {/* Background Halo */}
            <div className="absolute inset-0 bg-cyan-400/5 blur-[120px] rounded-full scale-125 pointer-events-none" />
         </div>

         <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative z-10">
            {[
              { label: 'Projected Growth', value: '+18.4%', desc: 'Annual scaling in cognitive tech infrastructure', gradient: 'from-cyan-400 to-blue-500', shadow: '0 0 40px rgba(0,243,255,0.12)' },
              { label: 'Clinical Accuracy', value: '99.4%', desc: 'Proprietary node-based data collection protocol', gradient: 'from-purple-400 to-pink-400', shadow: '0 0 40px rgba(168,85,247,0.12)' },
              { label: 'Network Nodes', value: '1,250+', desc: 'Active neural data validator nodes worldwide', gradient: 'from-blue-400 to-cyan-300', shadow: '0 0 40px rgba(59,130,246,0.12)' }
            ].map((stat, i) => (
               <motion.div
                  key={i}
                  initial={{ opacity: 0, y: 40 }}
                  ...
                  viewport={{ once: true }}
               >
                  <GlowCard key={i} className="p-10 text-center bg-white/[0.02] hover:bg-white/[0.05] border border-white/[0.05] hover:border-white/[0.12] transition-all duration-500 group" style={{ boxShadow: stat.shadow }}>
                     <div className="text-white/30 text-[10px] font-black tracking-[0.4em] uppercase mb-4 italic">{stat.label}</div>
                     <div className={`text-5xl font-black mb-6 tracking-tighter italic bg-gradient-to-r ${stat.gradient} bg-clip-text text-transparent transition-all duration-300`}>{stat.value}</div>
                     <div className="text-white/40 text-[11px] font-light tracking-[0.15em] uppercase leading-relaxed">{stat.desc}</div>
                  </GlowCard>
               </motion.div>
            ))}
         </div>
